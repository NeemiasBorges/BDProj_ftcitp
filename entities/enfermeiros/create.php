...
$nome = $data_nasc = $endereco = $bairro = $cidade = $estado = $telefone = $email = $cod_funcao = "";
$errors = [];
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (count($errors) == 0) {
        ...
        if ($con->query($sql) === TRUE) {
            header('Location: list.php'); // Redireciona para a lista de enfermeiros
            exit();
        } else {
            $errors[] = "Erro ao inserir: " . $con->error;
        }
    }
}
...
<head>
    ...
    <title>Adicionar Enfermeiro</title>
</head>
<body>
<div class="container">
